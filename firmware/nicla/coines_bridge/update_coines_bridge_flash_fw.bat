:: This script updates nicla board with Coines bridge firmware
...
%OPEN_OCD% -d2 -s %LIB_PATH% -f interface\cmsis-dap.cfg -c "transport select swd; adapter speed 1000" -f target\nrf52.cfg -c "telnet_port disabled; init; reset init; halt; adapter speed 10000;" -c "program coines_bridge_flash_firmware.hex" -c "reset run; shutdown"
