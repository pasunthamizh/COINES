 ...
 * @file    mcu_nicla.c
 * @date    May 10, 2022
 * @brief   COINES support file for NICLA MCU
 *
 */
...
uint8_t multi_io_map[COINES_SHUTTLE_PIN_MAX] = {
    [COINES_SHUTTLE_PIN_9] = 0xFF, 
    [COINES_SHUTTLE_PIN_14] = 0xFF, 
    [COINES_SHUTTLE_PIN_15] = 0xFF, 
    [COINES_SHUTTLE_PIN_16] = 0xFF, 
    [COINES_SHUTTLE_PIN_22] = 0xFF, 
    [COINES_SHUTTLE_PIN_8] = GPIO_16_SWO, 
    [COINES_SHUTTLE_PIN_20] = 0xFF, 
    [COINES_SHUTTLE_PIN_21] = GPIO_13, 
    [COINES_SHUTTLE_PIN_19] = 0xFF, 
    [COINES_SHUTTLE_PIN_7] = SPI0_SEN_CS_PIN, 
    0, 
    0, 
    0, 
    0, 
    ...
    0,
    /* Native APP3.0 pins */
    [COINES_MINI_SHUTTLE_PIN_1_4] = 0xFF,
    [COINES_MINI_SHUTTLE_PIN_1_5] = 0xFF,
    [COINES_MINI_SHUTTLE_PIN_1_6] = GPIO_13,
    [COINES_MINI_SHUTTLE_PIN_1_7] = 0xFF,
    [COINES_MINI_SHUTTLE_PIN_2_5] = 0xFF,
    [COINES_MINI_SHUTTLE_PIN_2_6] = GPIO_16_SWO,
    [COINES_MINI_SHUTTLE_PIN_2_1] = SPI0_SEN_CS_PIN,
    [COINES_MINI_SHUTTLE_PIN_2_3] = 0xFF,
    0,
    0,
    0,
	/*lint -e485 */
	...
	/*lint -e485 */
    [COINES_NICLA_CD_PIN] = BQ_CD,
    [COINES_MINI_SHUTTLE_PIN_2_7] = 0xFF,
    [COINES_MINI_SHUTTLE_PIN_2_8] = 0xFF,
...
#define RX_PIN_NUMBER_NICLA	  				9 					/* GPIO2 = p9 */
#define TX_PIN_NUMBER_NICLA   				20					/* GPIO1 = p20 */
#define UART_TX_BUF_SIZE 					256                 /**< UART TX buffer size. */
#define UART_RX_BUF_SIZE 					256                 /**< UART RX buffer size. */

/*uart baud rate*/
uint32_t uart_baud_rate = NRF_UART_BAUDRATE_115200;
/*Initializes the UART module*/
static uint32_t uart_init(uint32_t baud_rate);

/*For PMIC I2C communication*/
struct bq_dev pmic_dev = {  .i2c_address = BQ_DRV_ADDR, .cd_pin_state  = 1, .read = common_i2c_read,
                            .write = common_i2c_write, .delay_ms = common_delay_ms,
                            .cd_set = common_pmic_cd_set};
...
struct led_dev led_dev = { .i2c_address = LED_DRV_ADDR, .read = common_i2c_read,
                            .write = common_i2c_write, .delay_ms = common_delay_ms};

#define PMIC_FAULTS_READ_INTERVAL     APP_TIMER_TICKS(5000)  /* PMIC fault read interval (ticks). This value corresponds to 5 seconds. */
 ...
 */
void uart_error_handle_nicla(app_uart_evt_t * p_event)
{
...
static uint32_t uart_init(uint32_t baud_rate)
{
	...
	const app_uart_comm_params_t comm_params =
	{
			RX_PIN_NUMBER_NICLA,
			TX_PIN_NUMBER_NICLA,
			(uint32_t)RTS_PIN_NOT_CONNECTED,
			(uint32_t)CTS_PIN_NOT_CONNECTED,
	...
	/*Explicitly pulling up RX pin. The APP_UART_COMMUNICATION_ERROR will be triggering if the RX pin is left floating.*/
	nrf_gpio_cfg_input(RX_PIN_NUMBER_NICLA,NRF_GPIO_PIN_PULLUP);
...
void pmic_reading_vin_detection(void)
{
    ...
    if(BQ_OK == rslt)
    {
    	batt_status_in_milli_volts =(uint16_t) (((bat_vbbm * 4.2) / 100.0) * 1000.0); //Convert VBMON percentage to voltage => Volt = VBMON * VBATREG
...
int16_t coines_open_comm_intf(enum coines_comm_intf intf_type, void*arg)
{
	...
	if (NULL != intfconfig)
	{
#define COINES_NRF_BAUDRATE_MAP(coines_baudrate, nrf_baudrate)  \
		case  COINES_UART_BAUD_RATE_##coines_baudrate:         \
		uart_baud_rate = NRF_UART_BAUDRATE_##nrf_baudrate; \
		break \

		switch (intfconfig->uart_baud_rate)
		{
		COINES_NRF_BAUDRATE_MAP(1200, 1200);
		COINES_NRF_BAUDRATE_MAP(2400, 2400);

		COINES_NRF_BAUDRATE_MAP(4800, 4800);
		COINES_NRF_BAUDRATE_MAP(9600, 9600);
		COINES_NRF_BAUDRATE_MAP(14400, 14400);

		COINES_NRF_BAUDRATE_MAP(19200, 19200);
		COINES_NRF_BAUDRATE_MAP(28800, 28800);
		COINES_NRF_BAUDRATE_MAP(31250, 31250);
		COINES_NRF_BAUDRATE_MAP(38400, 38400);

		COINES_NRF_BAUDRATE_MAP(56000, 56000);
		COINES_NRF_BAUDRATE_MAP(57600, 57600);
		COINES_NRF_BAUDRATE_MAP(76800, 76800);

		COINES_NRF_BAUDRATE_MAP(115200, 115200);
		COINES_NRF_BAUDRATE_MAP(230400, 230400);
		COINES_NRF_BAUDRATE_MAP(250000, 250000);
		COINES_NRF_BAUDRATE_MAP(460800, 460800);
		COINES_NRF_BAUDRATE_MAP(921600, 921600);

		COINES_NRF_BAUDRATE_MAP(1000000, 1000000);
    ...
    /* Configure NFCT pins as GPIOs. UART RX is assigned to P0.9, same pin that is used as the antenna input for NFC by default*/
    reconfig_nfct_pin();

    if (NRF_SUCCESS != uart_init(uart_baud_rate))
    {
    	return COINES_E_INIT_FAILED;
    }
...
int _write(int fd, const char * p_char, int len)
{
    ...
    if ((fd == 1 || fd == 2))
    {
    	for (i = 0; i < len; i++)
    	{
    		UNUSED_VARIABLE(app_uart_put((uint8_t)*p_char++));
    	}
...
int _read(int fd, char * p_char, int len)
{
	if (fd == 0)
	{
		...
		return read_serial_buffer((uint8_t*)p_char, (uint16_t)len);
	}
	else if (fd >= 3 && fd < MAX_FILE_DESCRIPTORS + 3)
	{
