...
__version__ = '%d.%d.%d' % (MAJOR_VERSION, MINOR_VERSION, MICRO_VERSION)
...
class UserApplicationBoard(object):
    def __init__(self, path_to_coineslib=None):
        ...
        coines_get_version = self.LIB.coines_get_version
        coines_get_version.restype = ct.c_char_p
        self.coines_lib_version = coines_get_version().decode("utf-8") 
    ...
    def Read(self, bus, registerAddress, numberofReads=1, sensorInterfaceDetail=None):
        ...
        if self.sensorInterface == 'i2c':      # I2C bus configured
            ret = self.LIB.coines_read_i2c(bus, interface, int(registerAddress),\
                                                ct.byref(reg_data), int(numberofReads))
        elif self.sensorInterface == 'spi':                       # SPI bus configured
            if self.spi16bitEnable is False:
                ret = self.LIB.coines_read_spi(bus, interface, int(registerAddress),\
                                                ct.byref(reg_data), int(numberofReads))
            else:
                ret = self.LIB.coines_read_16bit_spi(bus, interface, registerAddress, \
    ...
    def Read16bitSPI(self, bus, registerAddress, numberofReads=2, sensorInterfaceDetail=None):
        ...
        if self.sensorInterface == 'spi': # SPI bus configured
            ret = self.LIB.coines_read_16bit_spi(bus, interface, int(registerAddress),\
    ...
    def Write(self, bus, registerAddress, registerValue, sensorInterfaceDetail=None):
        """
        ...
        if self.sensorInterface == 'i2c':      # I2C bus configured
            ret = self.LIB.coines_write_i2c(bus, interface, registerAddress, write_data, reg_length)
        elif self.sensorInterface == 'spi':  # SPI bus configured
            if self.spi16bitEnable is False:
                ret = self.LIB.coines_write_spi(bus, interface, registerAddress, write_data, reg_length)
            else:
                ret = self.LIB.coines_write_16bit_spi(bus, interface, registerAddress,\
