...
class CoinesBoard:
    """
    API to utilize the functions of coines header file through .dll/.so loaded
    """
    @staticmethod
    def __my_os_arch():
    ...
    def __coines_lib_for_mac(coinespy_path):
        if CoinesBoard.__my_os_arch() == '64bit':
            temp = os.path.join(coinespy_path, 'libcoines_64.dylib')
        else:  # 32bit
            temp = os.path.join(coinespy_path, 'libcoines_32.dylib')
        return temp
    ...
    def __init__(self, path_to_coines_lib: str = None):
        ...
        if self._lib is None:
            print(r"""
            Could not load coines lib shared library. Please initialize the
    ...
    def wrap_function(self, func_name: str, restype, argtypes):
        """ wrapping the func_name function equivalent to coines header file with same name
    ...
    def scan_ble_devices(self, scan_timeout_ms=0) -> Tuple[list, int]:
        ...
        self._lib.coines_scan_ble_devices.restype = ct.c_int8
