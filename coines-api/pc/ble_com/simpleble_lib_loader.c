...
#elif PLATFORM_MAC
#define DLL                "libsimpleble-c.dylib"
#else
#define DLL                "libsimpleble-c.so"
#endif
...
/* linux part */
#else
/* for macos and maybe old versions of glibc */
...
static bool load_library()
{
    if (lib_instance == NULL)
    {
         ...
         */
        /*lint -e{750}*/
        lib_instance = dlopen(DLL, RTLD_LAZY | RTLD_DEEPBIND);
        if (!lib_instance)
        {
            return false;
        }
...
static void *get_address(const char *function_name)
{
    ...
    return dlsym(lib_instance, function_name);
}
