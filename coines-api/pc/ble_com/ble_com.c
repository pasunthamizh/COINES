...
/*********************************************************************/
#define BLE_PERIPHERAL_LIST_SIZE  UINT8_C(100)
#define MIN_SCAN_TIMEOUT_MS       5000
#define COM_READ_BUFF             2048
#define BLE_ADDR_MAX_LEN          17
...
/* Function to check if a string matches the pattern of a MAC address */
static bool check_mac_address(const char *str)
{
    if (strlen(str) != BLE_ADDR_MAX_LEN)
    {

        /* The string is not the right length */
        return false;
    }

    for (size_t i = 0; i < BLE_ADDR_MAX_LEN; i++)
    {
        if (i % 3 == 2)
        {
            if (str[i] != ':' && str[i] != '-')
            {
                ...
                /* The string has an invalid separator character */
                return false;
            }
        }
        else
        {
            if (!isxdigit(str[i]))
            {
                ...
                /* The string has an invalid hexadecimal digit */
                return false;
            }
        }
    }

    /* The string matches the pattern of a MAC address */
    return true;
}
...
static int8_t get_ble_index_by_key(ble_index_key key, void *value)
{
    ...
    for (int8_t i = 0; i < peripheral_list_len; i++)
    {
        ...
        {
            case ADDRESS:
                if (strcmp(peripheral_address, (char *)value) == 0)
                {
                    return i;
                }
                ...
                break;
            case IDENTIFIER:
                if (strcmp(peripheral_identifier, (char *)value) == 0)
                {
                    return i;
                }
...
int8_t ble_connect(struct ble_peripheral_info *ble_config)
{
    ...
    if (!lib_instance && ble_scan_result == BLE_COM_OK)
    {
    ...
    else
    {
        ...
        if (ble_peripheral_index == -1 && check_mac_address(ble_config->ble_address))
        {
...
int8_t ble_read(void *buffer, uint32_t n_bytes, uint32_t *n_bytes_read)
{
    ...
    if (has_tx_notified && device_response_length >= n_bytes)
    {
