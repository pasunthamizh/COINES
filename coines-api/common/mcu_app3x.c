 ...
 * @file    mcu_app3x.c
 * @date    Aug 01, 2022
 * @brief   COINES support file for APP3.1 MCU
 *
 */
...
#define BLE_MTU                        230   /*Maximum payload for BLE com*/
...
uint8_t multi_io_map[COINES_SHUTTLE_PIN_MAX] = {
    ...
    [COINES_APP31_BUTTON_1] = SWITCH1, [COINES_APP31_BUTTON_2] = SWITCH2, [COINES_APP31_RESET_INT] = RESET_INT,
...
static void eeprom_init(void)
{
                          ...
                          EEPROM_READ_LEN) && app30_eeprom_read(0x01, (uint8_t *)&shuttle_id, 2))
    {
        ...
        /* Check the CS pin byte value,Should not be zero,if zero read failed. And check if shuttle id is in the range
        */
        if ((eeprom_read_buff[EEPROM_CS_BYTE_INDEX] != 0) && (shuttle_id > 0 && shuttle_id <= 0x1ff))
        {
...
#if defined(MCU_APP31)
    /* Configure the board switch and led */
    coines_set_pin_config(COINES_APP31_BUTTON_1, COINES_PIN_DIRECTION_IN, COINES_PIN_VALUE_HIGH);
...
int16_t coines_read_temp_data(float *temp_data)
{
    ...
    static bool is_temp_sensor_configured_once = false;
    ...
    /* If COINES_I2C_BUS_1 is already configured with other than BLE temperature sensor
     * then always return dummy data (0) from this callback with return value SUCCESS.
     */
    if (coines_is_i2c_enabled(temp_bus_intf) && !is_temp_sensor_configured_once)
    {
    ...
    /* Configure I2C bus 1 for BLE temperature sensor */
    if (!is_temp_sensor_configured_once)
    {
        ...
        is_temp_sensor_configured_once = true;
    }
 ...
 *
 * @param[out] bat_status_mv            :  Buffer to retrieve the battery status in %.
 *
...
int16_t coines_set_shuttleboard_vdd_vddio_config(uint16_t vdd_millivolt, uint16_t vddio_millivolt)
{
    ...
    else
    {
        ...
        /*We should desable the LDO output before changing the output voltage */
        nrf_gpio_pin_write(CHRG_LSCTRL, 0);
        nrf_gpio_pin_write(VDD_EN, 0);
        ldocfg.enable = BQ_LOAD_LDO_DISABLE;
