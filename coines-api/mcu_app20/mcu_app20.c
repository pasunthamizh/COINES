...
int16_t coines_timer_config(enum coines_timer_instance instance, void* handler)
{
	...
	if (instance < COINES_TIMER_INSTANCE_MAX)
	{
		if (!is_timer_enabled[instance])
		{
			...
			tc_init(TC0, timer_instance[instance], ul_tcclks | TC_CMR_CPCTRG);
...
int16_t coines_timer_start(enum coines_timer_instance instance, uint32_t timeout)
{
	...
	if (instance < COINES_TIMER_INSTANCE_MAX)
	{
		if (is_timer_enabled[instance])
		{
			desired_freq = (1000000 / timeout); /* timeout in microseconds */
			(void)tc_find_mck_divisor(desired_freq, ul_sysclk, &ul_div, &ul_tcclks, ul_sysclk);
			tc_init(TC0, timer_instance[instance], ul_tcclks | TC_CMR_CPCTRG);
			tc_write_rc(TC0, timer_instance[instance], (ul_sysclk / ul_div) / desired_freq);
