...
if __name__ == "__main__":
    ...
    # Read chip id
    chipid = BOARD.Read(bus, 0x00, 1 + SPI_DUMMY_BYTE, BMA400_IF_DETAIL)
    print('Chip ID accel: %#02x' % (chipid[0 + SPI_DUMMY_BYTE]))

    # Power up accelerometer
    pwr_before = BOARD.Read(bus, 0x19, 1 + SPI_DUMMY_BYTE, BMA400_IF_DETAIL)[0 + SPI_DUMMY_BYTE]
    print('Power settings before power-up (expected: 0x00): 0x%02x' % (pwr_before))
    # Change power mode from sleep to normal mode
    BOARD.Write(bus, 0x19, 0x02, BMA400_IF_DETAIL)
    time.sleep(.2)
    pwr_after = BOARD.Read(bus, 0x19, 1 + SPI_DUMMY_BYTE, BMA400_IF_DETAIL)[0 + SPI_DUMMY_BYTE]
    print('ACC: Power settings after power-up (expected: 0x02): 0x%02x' % (pwr_after))

    # Change to measurement range 4G
    acc_config1 = BOARD.Read(bus, 0x1A, 1 + SPI_DUMMY_BYTE, BMA400_IF_DETAIL)
    ...
    BOARD.Write(bus, 0x1A, acc_config1[0], BMA400_IF_DETAIL)
    time.sleep(.2)
    ...
    print('\n  ax\t   ay\t   az')
    for i in range(10):
        data = BOARD.Read(bus, 0x04, 6 + SPI_DUMMY_BYTE, BMA400_IF_DETAIL)
        if myif == 'spi':
            data.pop(0)
    ...
    data = BOARD.Read(bus, 0x11, 1 + SPI_DUMMY_BYTE, BMA400_IF_DETAIL)
