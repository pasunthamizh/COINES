...
def brd_read(sensor, reg, num_of_bytes):
    ...
    if (sensor.ident == 'accel') and (sensor.get_if_detail() == sensor.CSB):
        num_of_bytes += 1
    data = BOARD.Read(bus, reg & 0x7F, num_of_bytes, sensor.get_if_detail())
...
if __name__ == "__main__":
    ...
    #BOARD.Write(bus, 0x7C, [0, 4], bmi08x_accel.get_if_detail())
    #time.sleep(1)
    ...
    BOARD.Write(bus, 0x7C, 0, bmi08x_accel.get_if_detail())
    BOARD.Write(bus, 0x7D, 4, bmi08x_accel.get_if_detail())
    time.sleep(.05)
    pwr_after = brd_read(bmi08x_accel, 0x7C, 2)
    print('ACC: Power settings after power-up (expected: [0, 4]): %s' % (pwr_after))
    ...
    BOARD.Write(bus, 0x41, 0x00, bmi08x_accel.get_if_detail())
    time.sleep(.05)
    # Read back
