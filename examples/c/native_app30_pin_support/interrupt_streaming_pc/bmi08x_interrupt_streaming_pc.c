...
static void send_stream_settings()
{
    ...
    accel_stream_config.cs_pin = COINES_MINI_SHUTTLE_PIN_2_1;
    ...
    gyro_stream_config.cs_pin = COINES_MINI_SHUTTLE_PIN_2_5;
            ...
            if (rslt == COINES_SUCCESS)
            {
                ...
                for (idx = 0; idx < valid_sample_count; idx++)
                {
                    ...
                    if (accel_stream_config.int_timestamp)
                    {
                        ...
                        accel_time_stamp |=
                                            (uint64_t)bmi08x_accel_stream_buffer[buffer_index++]
                                            << 40;
                        accel_time_stamp |=
                                            (uint64_t)bmi08x_accel_stream_buffer[buffer_index++]
                                            << 32;
                    ...
                    printf(
                           "a_pc:%8d   a_t(us):%12"PRIu64" \t ax:%-5d ay:%-5d az:%-5d\n",
        ...
        if (bmi08xdev.gyro_cfg.power == BMI08X_GYRO_PM_NORMAL)
        {
            ...
            if (rslt == COINES_SUCCESS)
            {
                ...
                for (idx = 0; idx < valid_sample_count; idx++)
                {
                    ...
                    printf(
                           "g_pc:%8d   g_t(us):%12"PRIu64" \t gx:%-5d gy:%-5d gz:%-5d\n",
...
int main(int argc, char *argv[])
{
    ...
    init_bmi08x_sensor_driver_interface(BMI088_VARIANT);
    ...
    coines_start_stop_streaming(COINES_STREAMING_MODE_INTERRUPT,
    COINES_STREAMING_START);
